...
class ApplicationModelTest(TestCase):
    ...
    def test_full_address_removes_duplicate_protocol(self):
        ...
        test_cases = [
            ...
            (
                "https",
                "https://same-protocol.com",
                443,
                "https://same-protocol.com:443",
            ),
        ]
    ...
    def test_protocol_choices_validation(self):
        ...
        for protocol in valid_protocols:
            with self.subTest(protocol=protocol):
                app = Application(protocol=protocol, url="https://example.com", port=80)
    ...
    def test_invalid_protocol_choice(self):
        ...
        app = Application(
            protocol="invalid_protocol", url="https://example.com", port=80
        )
    ...
    def test_port_range_validation(self):
        ...
        for port in valid_ports:
            with self.subTest(port=port):
                app = Application(protocol="http", url="https://example.com", port=port)
    ...
    def test_port_boundary_values(self):
        ...
        app_min = Application(protocol="http", url="https://example.com", port=1)
        app_min.full_clean()  # Should not raise exception

        # Test maximum valid port
        app_max = Application(protocol="http", url="https://example.com", port=65535)
        app_max.full_clean()  # Should not raise exception

        # Test just below minimum (should fail)
        app_below_min = Application(protocol="http", url="https://example.com", port=0)
        with self.assertRaises(ValidationError):
            app_below_min.full_clean()

        # Test just above maximum (should fail)
        app_above_max = Application(
            protocol="http", url="https://example.com", port=65536
        )
        with self.assertRaises(ValidationError):
    ...
    def test_url_field_validation(self):
        ...
        for url in valid_urls:
            with self.subTest(url=url):
                app = Application(protocol="http", url=url, port=80)
                app.full_clean()  # Should not raise exception
    ...
    def test_field_max_lengths(self):
        ...
        app = Application(protocol=long_protocol, url="https://example.com", port=80)
        ...
        app = Application(protocol="http", url=long_url, port=80)
    ...
    def test_application_creation_and_retrieval(self):
        ...
        self.assertEqual(
            retrieved_app.protocol, self.valid_application_data["protocol"]
        )
    ...
    def test_common_port_numbers(self):
        ...
        for port in common_ports:
            with self.subTest(port=port):
                app = Application(protocol="tcp", url="https://service.com", port=port)
                app.full_clean()  # Should not raise exception
